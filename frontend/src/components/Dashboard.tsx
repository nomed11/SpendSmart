...
const Dashboard: React.FC = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const [categoryResponse, timeResponse] = await Promise.all([
                axios.get(`${import.meta.env.VITE_API_URL}/api/dashboard/spend-by-category`, {
                    headers: { Authorization: `Bearer ${token}` }
                }),
                axios.get(`${import.meta.env.VITE_API_URL}/api/dashboard/spend-over-time`, {
                    headers: { Authorization: `Bearer ${token}` }
                })
            ]);
    ...
    const handleNewRequest = (newRequest: any) => {
        ...
        fetchData(); // Refresh dashboard data after new request
    };
